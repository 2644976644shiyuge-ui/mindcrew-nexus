...
| 层级 | 技术选型 |
|:-----|:---------|
| 运行时 | **JDK 17（LTS）**（最低 17，可升 21） |
| 后端框架 | **Spring Boot 3.3.5** + Spring AI 1.1.4 |
...
```
知识库 (knowledge_collection) ── 1:N ── 文档 (kb_knowledge_base) ── 1:N ── 切片 (kb_chunk)
        ↑权限边界                            ↑内容载体                       ↑向量化单元
```

- **知识库（Collection）** —— 权限单元 + 对外接口单元
  - 创建 / 命名 / 图标 / 主题色 / 描述
  - 可见性：`public` 所有人 / `scoped` 按 ACL / `private` 仅创建者
  - 库级 ACL：按职位或部门授权（read / write / admin），部门级支持祖先继承
  - 库级 API Key：颁发给第三方系统，按月配额计费，可吊销
  - 缓存字段 `doc_count` / `total_chunks` 异步刷新
- **文档（Document）** —— 纯内容载体
  - `collection_id` 外键归属某个知识库（NULL = 散文档，仅创建者可见）
  - 上传 / 解析 / 切片 / 向量化 / 删除 / 移入库 / 重新分类
  - 权限**完全继承**所属知识库，不再单独配置可见性 / ACL / API Key
- **ACL（kb_acl）** —— 同表统一存储
  - `ref_type='collection'` 库级授权（推荐 · 商用主路径）
  - `ref_type='document'` 文档级授权（旧路径 · 兼容保留）

#### 上传与归类

- 支持 **PDF / DOCX / PPT / XLS / WPS / HTML / 音频 / 视频 / 微信聊天 / 图片 / 文本** 全格式
- 推荐流程：先建知识库 → 进库点「上传到此库」→ 文档自动归类继承权限
- 兼容流程：直接「上传文档」散传 → 之后从「所有文档」批量勾选 → 移入某库
- 全局上传队列 + 后台悬浮 Dock，切任何菜单都不打断
- 并发文件上传（默认 3 路并行）+ 文件夹拖拽 + 前端大小/格式预校验 + 失败重试
- OSS multipart 分片上传（>10MB 8 线程并行）· 大文件 3-5x 提速

#### 处理流水线

- 元数据自动提取：章节、页码、内容类型、音视频时间戳
- 视频：Qwen-VL-Max-Latest 原生理解（画面+字幕一次性产出 chunk）
- 音频：DashScope Paraformer-v2（逐句时间戳）
- 文档：DocumentExtractor + TextChunker（段落 + 句子 + 滑动窗口）
- 切片写入 MySQL + 向量写入 Milvus（双索引）
- 处理状态追踪：uploading → processing → ready / failed
- 支持失败文档重新处理
- **并发向量化**：8 线程并行调 text-embedding-v3，100 切片从 30s → 4-6s

#### 检索过滤（商用合规）

- 文档「能否被检索」= 它所属知识库「能否被访问」（继承自库级 visibility + ACL）
- chat / agent 调用 `KbAclService.listAccessibleKbIds(userId)`：
  1. 取用户可访问的全部知识库 id
  2. 展开成库内所有文档 id
  3. 加上散文档（自己创建的）+ 旧版文档级 ACL 命中
- 一次 SQL 取全集合 · 杜绝 N+1 · 越权检索物理不可能
...
| 关注点 | 归属 |
|---|---|
| 可见性（public / scoped / private） | 知识库 |
| 部门 / 职位 ACL | 知识库 |
| 对外 API Key | 知识库 |
| 上传 / 解析 / 切片 / 向量化 | 文档 |
| 文档归属（移入 / 移出库） | 文档 |

文档级权限完全继承所属知识库，不再单独配置 —— 一处管全部，商用合规。

---

## 数据库迁移

所有 schema 变更走 `sql/` 下的幂等迁移脚本，按顺序执行：

```bash
./sql/run.sh --all           # 按 sql/docker-init/00-init-all.sh 顺序全跑（幂等）
./sql/run.sh knowledge-collection-schema.sql   # 跑单个
```

**Docker 全新部署**：MySQL 容器首次启动自动执行 `sql/docker-init/00-init-all.sh`，按声明顺序 import 全部 SQL。

**关键迁移（任务 15 · 知识库两层架构）** —— [`sql/knowledge-collection-schema.sql`](sql/knowledge-collection-schema.sql)：

1. 新建 `knowledge_collection` 表（真正的"知识库"）
2. `kb_knowledge_base` 加 `collection_id` 外键（NULL = 散文档）
3. `kb_acl` 加 `ref_type` 字段（同表支持 `collection` / `document` 两级 ACL）
4. **自动迁移老数据**：按 `(user_id, category)` 分组，每组生成一个知识库，原文档自动归集
5. 缓存字段 `doc_count` / `total_chunks` 一次性算好

**升级老库一行命令搞定**：
```bash
./sql/run.sh knowledge-collection-schema.sql
```

迁移幂等 —— 重复执行不会重建表 / 不会重复归集，所有 ALTER 都通过 `information_schema` 判断是否已存在。

**配套迁移**：
- [`api-key-collection-migration.sql`](sql/api-key-collection-migration.sql)：API Key 加 `allowed_collection_ids`，老 key 自动按文档→所属库展开
- [`model-pricing-video-tts-migration.sql`](sql/model-pricing-video-tts-migration.sql)：补全视频 / TTS / 音色复刻定价
- [`usage-reconcile-schema.sql`](sql/usage-reconcile-schema.sql)：阿里 BSS 真实账单对账
- [`usage-cost-backfill.sql`](sql/usage-cost-backfill.sql)：cost_cny 历史回填
